...
function readInP(pInputAdd,kInputAdd,hMax)
    ...
    pData = pInfo(II,Ji,D,b,eff,B,p0,K,Pre,Succ,hMax);
    return pData;
end
...
function genData(filePath,Ωsize,hMax,dataSize = 1,pName = "test_P.csv",kName = "test_K.csv",ϕName = "test_Phi.csv",hName = "test_H.csv", dOnly = 0, hOnly = 0, saveOpt = 0)
    ...
    pData = readInP(pInputAdd,kInputAdd,hMax);
...
function genDataStrata(filePath,startaNo,sampleNo,hMax,dataSize = 1,pName = "test_P.csv",kName = "test_K.csv",ϕName = "test_Phi.csv",hName = "test_H.csv", saveOpt = 0)
    # under the file path, look for test_P, test_K and test_Phi
    pInputAdd = joinpath(filePath,pName);
    kInputAdd = joinpath(filePath,kName);
    ϕInputAdd = joinpath(filePath,ϕName);
