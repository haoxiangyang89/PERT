...
function subIntG(pData,disData,ω,xhat,that,Ghat)
    dDω = disData[ω];
    ...
    @constraint(sp, fixT[i in pData.II; Ghat[i,ω] == 0], t[i] == that[i]);
    @constraint(sp, boundT[i in pData.II; Ghat[i,ω] == 1], t[i] >= dDω.H);
    @constraint(sp, fixX[i in pData.II,j in pData.Ji[i]; Ghat[i,ω] == 0], x[i,j] == xhat[i,j]);
    ...
    @constraint(sp, durationConstr[k in pData.K; Ghat[k[1],ω] == 0], t[k[2]] - t[k[1]] >= pData.D[k[1]]*
        (1 - sum(pData.eff[k[1]][j]*x[k[1],j] for j in pData.Ji[k[1]])));
    @constraint(sp, durationConstr2[k in pData.K; Ghat[k[1],ω] == 1], t[k[2]] - t[k[1]] >= (pData.D[k[1]]+dDω.d[k[1]])*
        (1 - sum(pData.eff[k[1]][j]*x[k[1],j] for j in pData.Ji[k[1]])));
...
function ubCalG(pData,disData,Ω,xhat,that,Ghat)
    ...
    for ω in Ω
        cω = subIntG(pData,disData,ω,xhat,that,Ghat);
