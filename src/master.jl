function createMaster(pData,disData,Ω,Tmax = 999999)
    mp = Model(solver = GurobiSolver(OutputFlag = 0, IntFeasTol = 1e-7, FeasibilityTol = 1e-7));
    #mp = Model(solver = CplexSolver(CPX_PARAM_EPINT = 1e-7,CPX_PARAM_EPRHS = 1e-7,CPX_PARAM_SCRIND = 0));
    @variables(mp, begin
      θ[Ω] >= 0
