...
function solveMP_para_Share_noMW(data)
    ...
    mp = Model(solver = CplexSolver(CPX_PARAM_SCRIND = 0,CPX_PARAM_EPINT = 1e-8,
        CPX_PARAM_EPRHS = 1e-8,CPX_PARAM_THREADS = noTh,CPX_PARAM_CUTUP = ubCost,CPX_PARAM_TILIM = roundLimit));
...
function runPara_Share_noMW(treeList,cutList,ubCost,batchNo,noTh,noPa,ϵ = 1e-2,nSplit = 5)
...
function partSolve_BB_para_noMW(pData,disData,Ω,sN,MM,noThreads,batchNo,noTh,noPa,ϵ = 1e-2,nSplit = 5,roundLimit = 1000,ubGen = true)
    ...
    tic();
    tbest,xbest,ubCost,lbOverAll,timeIter,treeList = runPara_Share_noMW(treeList,cutList,ubCost,batchNo,noTh,noPa,ϵ,nSplit);
    decompTime = toc();
