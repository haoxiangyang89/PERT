# stratified version of the PERT problem
disData,Ω = autoUGenStrata("LogNormal",[log(35),0.5],nameD,dparams,50,10);
...
brInfo = precludeRelStrata(pData,Hω,Ωt);
...
@variable(mp, F[i in pData.II, ω in Ωt], Bin);
@variable(mp, G[i in pData.II, ω in Ωt], Bin);
...
@constraint(mp, GFixed1[i in pData.II,ω in Ωt; brInfo[i,ω] == 1],G[i,ω] == 1);
@constraint(mp, GFixed2[i in pData.II,ω in Ωt; brInfo[i,ω] == -1],F[i,ω] == 1);
...
@constraint(mp, tGnAnt[i in pData.II, ω in Ωt; brInfo[i,ω] == 0], t[i] <= Hω[ω] + G[i,ω]*min(tmaxD[i] - Hω[ω],M[i]));
@constraint(mp, tFnAnt[i in pData.II, ω in Ωt; brInfo[i,ω] == 0], t[i] >= Hω[ω] - F[i,ω]*(Hω[ω] - tminD[i]));
@constraint(mp, FGnAnt[i in pData.II, ω in Ωt], F[i,ω] + G[i,ω] == 1);
...
function innerCut(cb)
    ...
    if ((UB - LB)/LB > TOL)
        ...
        for ω in Ω
            ...
            @constraint(sp, tGbound1[i in pData.II; brInfo[i,ωt] == 1], tω[i] >= disData[ω].H);
            @constraint(sp, tGbound2[i in pData.II; brInfo[i,ωt] == -1], tω[i] == getvalue(t[i]));
            @constraint(sp, tGbound3[i in pData.II; brInfo[i,ωt] == 0], tω[i] >= disData[ω].H*getvalue(G[i,ωt]));
            @constraint(sp, xGbound1[i in pData.II, j in pData.Ji[i]; brInfo[i,ωt] == -1], xω[i,j] == getvalue(x[i,j]));
            ...
            @constraint(sp, tGnAnt1[i in pData.II; brInfo[i,ωt] == 0], tω[i] <= getvalue(t[i]) + getvalue(G[i,ωt])*Mω1[i,ω]);
            @constraint(sp, tGnAnt2[i in pData.II; brInfo[i,ωt] == 0], tω[i] >= getvalue(t[i]) - getvalue(G[i,ωt])*Mω2[i,ω]);
            @constraint(sp, xGnAnt1[i in pData.II, j in pData.Ji[i]; brInfo[i,ωt] == 0], xω[i,j] <= getvalue(x[i,j]) + getvalue(G[i,ωt]));
            @constraint(sp, xGnAnt2[i in pData.II, j in pData.Ji[i]; brInfo[i,ωt] == 0], xω[i,j] >= getvalue(x[i,j]) - getvalue(G[i,ωt]));

            @constraint(sp, sConstr1[i in pData.II, j in pData.Ji[i]; brInfo[i,ωt] == 0], sω[i,j] - xω[i,j] <= 0);
            @constraint(sp, sConstr2[i in pData.II, j in pData.Ji[i]; brInfo[i,ωt] == 0], sω[i,j] <= getvalue(G[i,ωt]));
            @constraint(sp, sConstr3[i in pData.II, j in pData.Ji[i]; brInfo[i,ωt] == 0], sω[i,j] - xω[i,j] + 1 >= getvalue(G[i,ωt]));
            @constraint(sp, sConstr4[i in pData.II, j in pData.Ji[i]; brInfo[i,ωt] == -1], sω[i,j] == 0);
            @constraint(sp, sConstr5[i in pData.II, j in pData.Ji[i]; brInfo[i,ωt] == 1], sω[i,j] - xω[i,j] == 0);
            ...
            @lazyconstraint(cb, θ[ω] >= vω + sum(πr[ω][i]*(t[i] - getvalue(t[i])) + γr[ω][i]*(G[i,ωt] - getvalue(G[i,ωt])) +
