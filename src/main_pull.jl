...
function buildTighten(pData,disData,Ω,cutSet,ub,lb,tmax,tmin)
    ...
    while ((UB - nCurrent.lbCost)/(nCurrent.lbCost) > TOL)&(termCond)
        ...
        function innerCut(cb)
            ...
            if contBool
                ...
                if length(LBList) % mIter == 0
                    # solve the new t bounds and brInfo
                    println(LBList[length(LBList)]);
                    bp = buildTighten(pData,disData,Ω,nCurrent.cutSet,UB,LBList[length(LBList)],nCurrent.tmaxD,nCurrent.tminD);
                    maxt = Dict();
                    mint = Dict();
                    for i in pData.II
        ...
        if mpStatus == :UserLimit
            ...
            node1,node2 = branchPull(pData,disData,Ω,node,tstar,nCurrent.lbCost,bestUB);
            if node1.state
                push!(nodeList,node1);
            end
            if node2.state
