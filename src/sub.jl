...
function subPull(pData,dDω,xhat,that,Ghatω,M = 9999999)
    ...
    # sp = Model(solver = CplexSolver(CPX_PARAM_SCRIND = 0));
    #sp = Model(solver = CplexSolver(CPX_PARAM_EPINT = 1e-8,CPX_PARAM_EPRHS = 1e-8,CPX_PARAM_SCRIND = 0));
    sp = Model(solver = GurobiSolver(OutputFlag = 0, IntFeasTol = 1e-8, FeasibilityTol = 1e-8));
    @variable(sp, 0 <= x[i in pData.II,j in pData.Ji[i]] <= 1);
    @variable(sp, t[i in pData.II] >= 0);
...
function sub_divTDualT2(pData,dDω,ωCurr,that,xhat,yhat,divSet,H,M,tcore,xcore,ycore,returnOpt = 0)
    ...
    if smpStatus != :Infeasible
        ...
        @variable(sp, λsG3[i in pData.II, j in pData.Ji[i]] >= 0);
        @variable(sp, λbudget <= 0);
        @variable(sp, λxSum[i in pData.II] <= 0);
        @variable(sp, λxub[i in pData.II, j in pData.Ji[i]] <= 0);
        @variable(sp, λGub[i in pData.II] <= 0);
        @variable(sp, λsub[i in pData.II, j in pData.Ji[i]] <= 0);
        @variable(sp, λFub[i in pData.II, par in 1:length(divSet[i])] <= 0);
        @variable(sp, λdur[k in pData.K] >= 0);
        ...
        # set up the initial solution for sp
        # setvalue(λbudget,getdual(smp[:budgetConstr]));
        # for i in pData.II
        #     setvalue(λHG1[i],getdual(smp[:GCons1][i]));
        #     setvalue(λHG1[i],getdual(smp[:GCons2][i]));
        #     setvalue(λGy1[i],getdual(smp[:GFixed0][i]));
        #     setvalue(λGy2[i],getdual(smp[:GFixed1][i]));
        #     setvalue(λtG1[i],getdual(smp[:tGbound][i]));
        #     setvalue(λtG2[i],getdual(smp[:tFnAnt1][i]));
        #     setvalue(λtG3[i],getdual(smp[:tFnAnt2][i]));
        #     setvalue(λxub[i],getdual(smp[:xConstr][i]));
        #     for j in pData.Ji[i]
        #         setvalue(λxG1[i,j],getdual(smp[:xFnAnt1][i,j]));
        #         setvalue(λxG2[i,j],getdual(smp[:xFnAnt2][i,j]));
        #         setvalue(λsG1[i,j],getdual(smp[:xGlin1][i,j]));
        #         setvalue(λsG2[i,j],getdual(smp[:xGlin2][i,j]));
        #         setvalue(λsG3[i,j],getdual(smp[:xGlin3][i,j]));
        #     end
        #     for par in 1:length(divSet[i])
        #         setvalue(λFG1[i,par],getdual(smp[:GyRelax1][i,par]));
        #         setvalue(λFG2[i,par],getdual(smp[:GyRelax2][i,par]));
        #         setvalue(λFG3[i,par],getdual(smp[:GyRelax3][i,par]));
        #     end
        # end
        # for k in pData.K
        #     setvalue(λGG[k],getdual(smp[:GSuccessors][k[1],k[2]]));
        #     setvalue(λdur[k],getdual(smp[:durationConstr][k]));
        # end
        ...
        # x constraint
        @constraint(sp, xConstr[i in pData.II, j in pData.Ji[i]], -λxG1[i,j] - λxG2[i,j] +
            λsG2[i,j] + λsG3[i,j] - pData.b[i][j]*λbudget - λxSum[i] - λxub[i,j] - sum(pData.D[i]*pData.eff[i][j]*λdur[k] for k in pData.K if k[1] == i) >= 0);
        # G constraint
        @constraint(sp, gConstr[i in pData.II], sum(λFG1[i,par] + λFG3[i,par] for par in 1:length(divSet[i])) - dDω.H*(λHG1[i] - λHG2[i]) -
            λGy1[i] - λGy2[i] + sum(λGG[k] for k in pData.K if k[2] == i) - sum(λGG[k] for k in pData.K if k[1] == i) + λtG1[i]*dDω.H -
            λtG2[i]*M[i] + λtG3[i]*M[i] + sum(-λxG1[i,j] + λxG2[i,j] + λsG1[i,j] + λsG3[i,j] for j in pData.Ji[i]) +
            sum(dDω.d[i]*λdur[k] for k in pData.K if k[1] == i) - λGub[i] >= 0);
        # Gy constraint
        @constraint(sp, FConstr[i in pData.II, par in 1:length(divSet[i])], -λFG1[i,par] - λFG2[i,par] - λFG3[i,par] +
            H[divSet[i][par].endH]*λHG1[i] - H[divSet[i][par].startH]*λHG2[i] - λFub[i,par] >= 0);
        # s constraint
        @constraint(sp, sConstr[i in pData.II, j in pData.Ji[i]], -λsG1[i,j] - λsG2[i,j] - λsG3[i,j] -
            sum(dDω.d[i]*pData.eff[i][j]*λdur[k] for k in pData.K if k[1] == i) - λsub[i,j] >= 0);
        ...
        @expression(sp, corePoint, sum(sum(yhat[i,par]*λFG2[i,par] + (yhat[i,par] - 1)*λFG3[i,par] for par in 1:length(divSet[i])) for i in pData.II) +
            ...
            sum(λGy1[i]*(sum(yhat[i,par] for par in 1:length(divSet[i]) if dDω.H <= H[divSet[i][par].startH])) +
                λGy2[i]*(1 - sum(yhat[i,par] for par in 1:length(divSet[i]) if dDω.H >= H[divSet[i][par].endH])) for i in pData.II) +
            sum(that[i]*(λtG2[i] + λtG3[i]) + sum(xhat[i,j]*(λxG1[i,j] + λxG2[i,j]) for j in pData.Ji[i]) for i in pData.II) -
            sum(sum(λsG3[i,j] for j in pData.Ji[i]) for i in pData.II) + pData.B*λbudget + sum(λxSum[i] for i in pData.II) +
            sum(pData.D[k[1]]*λdur[k] for k in pData.K) + sum(λGub[i] for i in pData.II) + sum(λFub[i,par] for i in pData.II for par in 1:length(divSet[i])) +
            + sum(λsub[i,j] + λxub[i,j] for i in pData.II for j in pData.Ji[i]));

        # objective function of the binary feasible solution should be the same
        @constraint(sp, binaryTight, corePoint >= (1- 1e-5)*vhat);

        # optimize the fractional solution's objective
        @expression(sp, actualCore, sum(sum(ycore[i,par]*λFG2[i,par] + (ycore[i,par] - 1)*λFG3[i,par] for par in 1:length(divSet[i])) for i in pData.II) +
            sum(λHG1[i]*(dDω.H - tcore[i]) + λHG2[i]*(-tcore[i] + sum(ycore[i,par]*H[divSet[i][par].startH] for par in 1:length(divSet[i]))) for i in pData.II) +
            sum(λGy1[i]*(sum(ycore[i,par] for par in 1:length(divSet[i]) if dDω.H <= H[divSet[i][par].startH])) +
                λGy2[i]*(1 - sum(ycore[i,par] for par in 1:length(divSet[i]) if dDω.H >= H[divSet[i][par].endH])) for i in pData.II) +
            sum(tcore[i]*(λtG2[i] + λtG3[i]) + sum(xcore[i,j]*(λxG1[i,j] + λxG2[i,j]) for j in pData.Ji[i]) for i in pData.II) -
...
function sub_divTT(pData,dDω,ωCurr,that,xhat,yhat,divSet,H,M,returnOpt = 0)
    ...
    for i in pData.II
        for par in 1:length(divSet[i])
            if (ωCurr >= divSet[i][par].startH) & (ωCurr < divSet[i][par].endH)
                Hqt[i] = par;
            end
        end
    end
    ...
    @constraint(smp, xGlin2[i in pData.II, j in pData.Ji[i]], s[i,j] <= x[i,j]);
    @constraint(smp, xGlin3[i in pData.II, j in pData.Ji[i]], s[i,j] >= x[i,j] - 1 + G[i]);
    @constraint(smp, xGlin4[i in pData.II, j in pData.Ji[i]; !(ωCurr in [divSet[i][par].startH for par in 1:length(divSet[i])])],
        s[i,j] <= G[i] + (that[i] - dDω.H - sum(H[divSet[i][par].endH]*yhat[i,par] for par in 1:length(divSet[i]) if par != Hqt[i]))/(H[divSet[i][Hqt[i]].endH] - dDω.H)*x[i,j] -
        (that[i] - dDω.H - sum(H[divSet[i][par].endH]*yhat[i,par] for par in 1:length(divSet[i]) if par != Hqt[i]))/(H[divSet[i][Hqt[i]].endH] - dDω.H));
    @constraint(smp, xGlin5[i in pData.II, j in pData.Ji[i]; !(ωCurr in [divSet[i][par].startH for par in 1:length(divSet[i])])],
        s[i,j] <= (that[i] - H[divSet[i][Hqt[i]].startH]*yhat[i,Hqt[i]])/(dDω.H - H[divSet[i][Hqt[i]].startH])*x[i,j]);
    @constraint(smp, xGlin6[i in pData.II, j in pData.Ji[i]; !(ωCurr in [divSet[i][par].startH for par in 1:length(divSet[i])])],
        s[i,j] >= G[i] + (that[i] - H[divSet[i][Hqt[i]].startH]*yhat[i,Hqt[i]])/(dDω.H - H[divSet[i][Hqt[i]].startH])*x[i,j] -
        (that[i] - H[divSet[i][Hqt[i]].startH]*yhat[i,Hqt[i]])/(dDω.H - H[divSet[i][Hqt[i]].startH]));
    @constraint(smp, xGlin7[i in pData.II, j in pData.Ji[i]; !(ωCurr in [divSet[i][par].startH for par in 1:length(divSet[i])])],
        s[i,j] >= (that[i] - dDω.H - sum(H[divSet[i][par].endH]*yhat[i,par] for par in 1:length(divSet[i]) if par != Hqt[i]))/(H[divSet[i][Hqt[i]].endH] - dDω.H)*x[i,j]);
    ...
    if smpStatus == :Optimal
        ...
        if returnOpt == 0
            ...
            for i in pData.II
                ...
                for j in pData.Ji[i]
                    λdict[i,j] = (getdual(smp[:xFnAnt1][i,j]) + getdual(smp[:xFnAnt2][i,j]));
                    if !(ωCurr in [divSet[i][par1].startH for par1 in 1:length(divSet[i])])
                        πdict[i] += getdual(smp[:xGlin4][i,j])/(H[divSet[i][Hqt[i]].endH] - dDω.H)*(getvalue(smp[:x][i,j]) - 1) +
                        getdual(smp[:xGlin5][i,j])/(dDω.H - H[divSet[i][Hqt[i]].startH])*getvalue(smp[:x][i,j]) +
                        getdual(smp[:xGlin6][i,j])/(dDω.H - H[divSet[i][Hqt[i]].startH])*(getvalue(smp[:x][i,j]) - 1) +
                        getdual(smp[:xGlin7][i,j])/(H[divSet[i][Hqt[i]].endH] - dDω.H)*getvalue(smp[:x][i,j]);
                    end
                end
                for par in 1:length(divSet[i])
                    γdict[i,par] = H[divSet[i][par].startH]*getdual(smp[:GCons2][i]) +
                    ...
                    if !(ωCurr in [divSet[i][par1].startH for par1 in 1:length(divSet[i])])
                        if par == Hqt[i]
                            for j in pData.Ji[i]
                                γdict[i,par] -= getdual(smp[:xGlin5][i,j])*H[divSet[i][par].startH]/(dDω.H - H[divSet[i][Hqt[i]].startH]);
                                γdict[i,par] -= getdual(smp[:xGlin6][i,j])*H[divSet[i][par].startH]/(dDω.H - H[divSet[i][Hqt[i]].startH])*(getvalue(smp[:x][i,j]) - 1);
                            end
                        else
                            for j in pData.Ji[i]
                                γdict[i,par] -= getdual(smp[:xGlin7][i,j])*H[divSet[i][par].endH]/(H[divSet[i][Hqt[i]].endH] - dDω.H);
                                γdict[i,par] -= getdual(smp[:xGlin4][i,j])*H[divSet[i][par].endH]/(H[divSet[i][Hqt[i]].endH] - dDω.H)*(getvalue(smp[:x][i,j]) - 1);
                            end
                        end
                    end
                    if dDω.H <= H[divSet[i][par].startH]
...
function sub_divT_rev(pData,dDω,ωCurr,that,xhat,yhat,divSet,H,M,returnOpt = 0)
    # using the reviewer's suggestion without linearizing the bilinear term
    #global GUROBI_ENV;
    sp = Model(solver = GurobiSolver(GUROBI_ENV,IntFeasTol = 1e-8,OutputFlag = 0));
    @variable(sp, 0 <= x[i in pData.II,j in pData.Ji[i]] <= 1);
    @variable(sp, t[i in pData.II] >= 0);
    ...
    # add the basic sub problem constraints
    # obtain the current par for each activity
    currentPart = Dict();
    for i in pData.II
        for par in 1:length(divSet[i])
            if (dDω.H < H[divSet[i][par].endH]) & (dDω.H >= H[divSet[i][par].startH])
                currentPart[i] = par;
            end
        end
    end
    @constraint(sp, GCons1[i in pData.II], sum((H[divSet[i][par].endH] - dDω.H + H[divSet[i][currentPart[i]].endH])*yhat[i,par] for par in 1:(currentPart[i]-1)) +
                                                    (dDω.H - H[divSet[i][currentPart[i]].endH])*(1 - G[i]) +
                                                    sum(H[divSet[i][par].endH]*yhat[i,par] for par in currentPart[i]:length(divSet[i])) >= that[i]);
    @constraint(sp, GCons2[i in pData.II],sum(H[divSet[i][par].startH]*yhat[i,par] for par in 1:currentPart[i]) +
                                                    (dDω.H - H[divSet[i][currentPart[i]].startH])*G[i] +
                                                    sum((H[divSet[i][par].startH] - dDω.H + H[divSet[i][currentPart[i]].startH])*yhat[i,par] for par in (currentPart[i]+1):length(divSet[i])) <= that[i]);
    #@constraint(sp, GCons1[i in pData.II],dDω.H*G[i] - sum(H[divSet[i][par].endH]*Gy[i,par] for par in 1:length(divSet[i])) <= dDω.H - that[i]);
    #@constraint(sp, GCons2[i in pData.II],sum(H[divSet[i][par].startH]*Gy[i,par] for par in 1:length(divSet[i])) - dDω.H*G[i] >= -that[i] + sum(yhat[i,par]*H[divSet[i][par].startH] for par in 1:length(divSet[i])));
    @constraint(sp, GFixed0[i in pData.II],G[i] >= sum(yhat[i,par] for par in 1:length(divSet[i]) if dDω.H <= H[divSet[i][par].startH]));
    @constraint(sp, GFixed1[i in pData.II],G[i] <= 1 - sum(yhat[i,par] for par in 1:length(divSet[i]) if dDω.H >= H[divSet[i][par].endH]));
        ...
        if spStatus == :Optimal
            ...
            for i in pData.II
                ...
                for par in 1:length(divSet[i])
                    ...
                    if par <= currentPart[i] - 1
                        γdict[i,par] -= (H[divSet[i][par].endH] - dDω.H + H[divSet[i][currentPart[i]].endH])*getdual(sp[:GCons1][i]);
                        γdict[i,par] -= H[divSet[i][par].startH]*getdual(sp[:GCons2][i]);
                    elseif par == currentPart[i]
                        γdict[i,par] -= H[divSet[i][par].endH]*getdual(sp[:GCons1][i]);
                        γdict[i,par] -= H[divSet[i][par].startH]*getdual(sp[:GCons2][i]);
                    else
                        γdict[i,par] -= H[divSet[i][par].endH]*getdual(sp[:GCons1][i]);
                        γdict[i,par] -= (H[divSet[i][par].startH] - dDω.H + H[divSet[i][currentPart[i]].startH])*getdual(sp[:GCons2][i]);
                    end
...
function sub_divT_after(pData,dDω,ωCurr,that,xhat,yhat,divSet,H,M,returnOpt = 0)
    #global GUROBI_ENV;
    sp = Model(solver = GurobiSolver(GUROBI_ENV,IntFeasTol = 1e-8,OutputFlag = 0));
    @variable(sp, 0 <= x[i in pData.II,j in pData.Ji[i]] <= 1);
    @variable(sp, t[i in pData.II] >= 0);
