...
function createPar(pData,disData,Ω,partCurrentTemp,partDetTemp,μp)
    ...
    for i in pData.II
        ...
        for pci in 1:length(partCurrentTemp[i])
            ...
            μFCurrent = sum(μp[i,ω] for ω in partCurrentTemp[i][pci]);
            μFBest = μFCurrent;
            μFBestInd = length(partCurrentTemp[i][pci]);
            if partDet[i][pci] == 0
                for ind in 1:(length(partCurrentTemp[i][pci]) - 1)
                    firstP = max(0,sum(μp[i,ωi] for ωi in partCurrentTemp[i][pci][1:ind]));
                    secondP = max(0,sum(μp[i,ωi] for ωi in partCurrentTemp[i][pci][(ind+1):length(partCurrentTemp[i][pci])]));
