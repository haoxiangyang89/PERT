...
\section{Problem Formulation} \label{sec:formulation}
	 Suppose the activity network is represented by a directed graph , where the set of activities is denoted by \(I\) and their precedence relationship is characterized by \(\mathcal{A}\). The arc \((i,j) \in \mathcal{A}\) indicates that activity \(i\) has to be finished before the start of activity \(j\).  \\
	\newline
	For each activity \(i \in I\), the nominal length \(D_i\) is given.  and the problem can be formulated as follows:
